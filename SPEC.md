...
This document is the product definition and source of truth for `mural`.
...
- Self-describing: `mural` invoked with no arguments must print a rich bootstrap guide that teaches the JSONL workflow and exit.
...
The default machine-local home root is `~/.mural/`. The home root may be overridden via the `MURAL_HOME` environment variable for testing or constrained environments.
...
- `mural export` — export PNG to a specified path
- `mural watch` — open a read-only watcher window

**Convenience commands** (single-operation shortcuts):

- `mural draw` — create a single object
- `mural edit` — modify a single object
- `mural delete` — delete a single object
- `mural undo` — undo the most recent operation

The convenience commands are thin wrappers. Every operation they perform is also available through `mural run`.
...
Object IDs may be omitted from draw operations; `mural` will generate sequential IDs and include them in the results.
...
All convenience commands support `--json` for structured output, following the same output contract as a single-operation `mural run --json` call.
...
`mural run` is the primary agent interface.
...
If callers omit object IDs for draw operations, `mural` generates sequential IDs. The assigned IDs are returned in the results and recorded in the canonical command log.

### 12.3 Output format

See §9.3 for the full `mural run --json` output specification.

### 12.4 End-to-end agent example

```bash
# 1. Create a session
SESSION=$(mural new --json | python -c "import sys,json; print(json.load(sys.stdin)['session_path'])")

# 2. Draw a diagram via JSONL batch
cat <<'EOF' | mural run --session "$SESSION" --json
...
mural inspect --session "$SESSION" --json

# 4. Export to a shareable PNG
mural export --session "$SESSION" --out ./diagram.png
```
...
The lock implementation is file-based with PID tracking. Machine-local lock artifacts live under `~/.mural/`, not in the session directory. Stale locks from dead processes are automatically reclaimed.
